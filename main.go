...
func checkNextcloud(serverURL string, ncToken string, critSpace int, warnSpace int, diskLocation string) {
	...
	if err := syscall.Statfs(diskLocation, &stat); err != nil {
		fmt.Printf("CRITICAL - Failed to get disk usage for %s: %v\n", diskLocation, err)
		os.Exit(2)
	}
	...
	status := "OK"
	exitCode := 0
	if freePercent <= float64(critSpace) {
		status = "CRITICAL"
		exitCode = 2
	} else if freePercent <= float64(warnSpace) {
		status = "WARNING"
		exitCode = 1
	}
	...
	metrics := map[string]interface{}{
		...
		"free_space_percent":        math.Round(freePercent*100) / 100,
...
func main() {
	...
	critical := flag.Int("c", 5, "Critical threshold for free space in percent")
	warning := flag.Int("w", 25, "Warning threshold for free space in percent")
	disklocation := flag.String("d", "/", "Disk location to check for free space")
	...
	checkNextcloud(*server, *token, *critical, *warning, *disklocation)
}
